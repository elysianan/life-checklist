...
const DatePickerManager = {
  ...
  open(currentDateStr, onConfirm, allowFuture = false) {
    ...
    const maxYear = allowFuture ? currentYear + 100 : currentYear;

    this._fill(document.getElementById('dp-year'), this._yearRange(maxYear), d.getFullYear());
    ...
    // 遮罩刚显示时立即设置 scrollTop 可能不生效，下一帧重设以定位到默认日期
    requestAnimationFrame(() => {
      this._fill(yearEl, this._yearRange(maxYear), d.getFullYear());
  ...
  _yearRange(maxYear) {
    if (!Number.isFinite(maxYear)) maxYear = new Date().getFullYear();
    const r = [];
